...
imageSample <- sample_n(directoryListing, 1000)
...
imagesDF <- lapply(seq_along(imagesHogged), function(index) {
  df <- data.frame(image = index, class = imageSample$class[[index]], x = 1:54, value = imagesHogged[[index]])
  return(df)
})
imagesHoggedDF <- bind_rows(imagesHoggedDF)
imagesHoggedDF <- imagesHoggedDF %>% spread(x, value)
imageClassKey <- imagesHoggedDF$class
features <- imagesHoggedDF[,(3:50)]
```
